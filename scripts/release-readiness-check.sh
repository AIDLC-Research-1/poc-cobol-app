...
# COBOL build remains manual-gate; this check does not assert a build/run
# result.
#
# NOTE: also exists as a workflow_call reusable workflow in
# AIDLC-Research-1/.github-private; cross-repo resolution of that failed
# at runtime, so this local copy is what actually runs.
set -euo pipefail
...
if [ -z "$MISSING" ]; then
  VERDICT="PASS: spec, design, and security-review artefacts are all present for ${JIRA_KEY}. Build remains manual-gate (GnuCOBOL not present in CI)."
else
